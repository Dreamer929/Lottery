...
@interface SevenLottyViewController ()
...
@implementation SevenLottyViewController
...
- (void)viewDidLoad {
    ...
    if (self.flag == 5) {
        self.navigationItem.title = @"七乐彩";
    }else{
        self.navigationItem.title = @"七星彩";
    }
...
-(void)createUI{
    ...
    self.scrollview.contentSize = CGSizeMake(DREAMCSCREEN_W, DREAMCSCREEN_H + 150);
    ...
    if ([MXAppConfig shareInstance].isLogin) {
        if (self.redBallCount.count>=7) {
            ...
            UIAlertAction *go = [UIAlertAction actionWithTitle:@"YES" style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {
                ...
                if (self.flag == 5) {
                    parameter.lotteryType = @"qlc";
                    parameter.qihao = @"2017087";
                }else{
                    parameter.lotteryType = @"qxc";
                    parameter.qihao = @"20172088";
                }
